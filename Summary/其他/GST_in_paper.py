...
class STree:
    ...
    def __repr__(self):
        def state_desc(state: State):
            ...
            return self.string[state.left:] if state.right == float('inf') else self.string[state.left:state.right+1]
    ...
    def _update(self, active_state, left, idx):
        ...
        while not is_end_point:
            ...
            split_state.transition[self.string[idx]] = new_state
    ...
    def _test_and_split(self, active_state, left, right, char):
        ...
        if char == self.string[next_char_idx]:
